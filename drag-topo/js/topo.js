...
*/
	...
	instance.bind("connectionDragStop", function(info) {//点击连接线、overlay、label提示删除连线 + 不能以自己作为目标元素
	  ...
	  /*  if(info.sourceId == info.targetId){
		...
		}else if(info.sourceId.substring(0,6) == info.targetId.substring(0,6)){
			$.popupTips('不能主机链接主机');
             instance.detach(info); 
		}else if(info.targetId.substring(0,6)=='uiscre'){
              $.popupTips('只能云主机连接交换机');
              instance.detach(info); 
		}
		else{
			info.unbind('click');
    ...
    //页面放大缩小
     /* $('#topocontent').bind('mousewheel', function(event, delta, deltaX, deltaY) {
            if (delta == 1) {
                 $('#topocontent').css({"transform":"scale(0.5,0.5)","-webkit-transition":"all 2s"})
             }else  if (window.console && console.log) {
                 console.log(delta, deltaX, deltaY);
            }
        });
        // using the event helper
        $('#topocontent').mousewheel(function(event, delta, deltaX, deltaY) {
            if (delta != 1) {
                 $('#topocontent').css({"transform":"scale(1.5,1.5)","-webkit-transition":"all 2s"})
             }else     if (window.console && console.log) {
                 console.log(delta, deltaX, deltaY);
            }
        });*/
